...
import ListingItemfeatured from "../components/Listings/ListingItemfeatured";
...
export default function PropertyListing() {
  const [listings, setListings] = useState(FeaturedData);
  ...
  const applyFilters = () => {
    let filteredListings = FeaturedData;
                  ...
                  <div className="m-2 mb-9 relative top-[40px]">
                    <ul className="grid xxxs:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 ">
                      {listingsToDisplay.map((listing, index) => (
                        <ListingItemfeatured key={index} listing={listing} />
                      ))}
                    </ul>
                  </div>
                ) : (
